...
import { AxisState } from './BaseGrid';
import { pad, range, almost, scale, clamp, isMultiple, lg } from './mumath';

export abstract class AxisScaling {
  steps: number[];
  distance: number;
  abstract lines: (state: AxisState) => number[];
  abstract lineColor: (state: AxisState) => number[];
  abstract ticks: (state: AxisState) => number[];
  abstract labels: (state: AxisState) => string[];
}

export class LinearScaling extends AxisScaling {
  steps: [1, 2, 5];

  distance: 20;

  lines = (state: AxisState): number[] => {
